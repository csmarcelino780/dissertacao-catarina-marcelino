...
# Create a writer
with pd.ExcelWriter('MLR_scores_verao_solar.xlsx') as writer:
...
# Create a writer
with pd.ExcelWriter('MLR_scores_verao_load.xlsx') as writer:
    # Write each DataFrame to a different sheet
    method1_df.to_excel(writer, sheet_name='Method1', index=False)
    real_values_df.to_excel(writer, sheet_name='Real_Values', index=False)
...
# # Create a writer
# with pd.ExcelWriter('MLR_scores_verao.xlsx') as writer:
#     # Write each DataFrame to a different sheet
#     method1_df.to_excel(writer, sheet_name='Method1', index=False)
...
# # Create a writer
# with pd.ExcelWriter('Montly_MLR_scores_verao.xlsx') as writer:
#     # Write each DataFrame to a different sheet
#     method1_df.to_excel(writer, sheet_name='Method1', index=True)
#     method2_df.to_excel(writer, sheet_name='Method2', index=True)

# # Create a writer
# with pd.ExcelWriter('MLR_weekday_scores_verao.xlsx') as writer:
