# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 11:52:39 2023
    ...
    for day in range(1, 31):
        for hour in range(24):  
            ...
            if prediction_datetime > end_date or (hour >= 12 and hour < 24 and day > 1):
                ...
                # Make the hour-ahead forecast
                forecasted_load = linear_regression.predict(forecast_features.values.reshape(1, -1))
...
#Linear Regression model for solar data
linear_regression_solar = LinearRegression()
...
for train_index_solar, test_index_solar in tscv.split(X_solar):
    ...
    # Fit the model 
    linear_regression_solar.fit(X_train_solar[all_features_solar], y_train_solar)

    # Forecast 
    forecast_features_solar = X_test_solar[all_features_solar].iloc[-24:]
    forecasted_solar = linear_regression_solar.predict(forecast_features_solar)
    ...
    for day in range(1, 31):
        for hour in range(24):  
            ...
            if prediction_datetime_solar > end_date_solar or (hour >= 12 and hour < 24 and day > 1):
                ...
                # Make the hour-ahead forecast for solar data
                forecasted_solar_values = linear_regression_solar.predict(forecast_features_solar.values.reshape(1, -1))
...
real_solar_prediction_days = solar_real_resampled.loc[solar_real_resampled.index.month.isin([11,12])]['PV25']  # Assuming solar data is used for solar_real
...
#  Linear Regression model
linear_regression_net = LinearRegression()
...
for train_index, test_index in tscv_net.split(X_net):
    ...
    # Fit the model 
    linear_regression_net.fit(X_train_net[all_features_net], y_train_net)

    # Forecast
    forecast_features_net = X_test_net[all_features_net].iloc[-24:]
    forecasted_load_net = linear_regression_net.predict(forecast_features_net)
    ...
    for day in range(1, 31):
        for hour in range(24):  
            ...
            if prediction_datetime > end_date or (hour >= 12 and hour < 24 and day > 1):
                ...
                # Make the hour-ahead forecast for net load
                forecasted_net_load = linear_regression_net.predict(forecast_features_net.values.reshape(1, -1))
...
# Create a writer
with pd.ExcelWriter('MLR_scores_inverno_solar.xlsx') as writer:
...
# Create a writer
with pd.ExcelWriter('MLR_scores_inverno_load.xlsx') as writer:
    # Write each DataFrame to a different sheet
    method1_df.to_excel(writer, sheet_name='Method1', index=False)
    real_values_df.to_excel(writer, sheet_name='Real_Values', index=False)
...
# # Create a writer
# with pd.ExcelWriter('MlR_scores_inverno.xlsx') as writer:
#     # Write each DataFrame to a different sheet
#     method1_df.to_excel(writer, sheet_name='Method1', index=False)
...
# # Create a writer
# with pd.ExcelWriter('Montly_MLR_scores_inverno.xlsx') as writer:
#     # Write each DataFrame to a different sheet
...
# # Create a writer
# with pd.ExcelWriter('MLR_weekday_scores_inverno.xlsx') as writer:
